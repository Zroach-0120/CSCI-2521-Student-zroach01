...
{
    cout << "Each guess must be an isogram " << WORD_LEN
        << " letters in length. Please try again." << endl;
}
...
void Engine::incorrectGuess(string hiddenWord, string playerGuess)
{
    ...
    cout << "Sorry, " << playerGuess
        << " is not the hidden word. You have "
        << catsAndCougars->getCats()
        << " Cats and "
        << catsAndCougars->getCougars()
        << " Cougars." << endl;
}